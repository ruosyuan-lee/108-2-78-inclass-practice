...
```{r ans22}
taiwan_times <- c(ymd_hms(taiwan_time,tz="Asia/Taipei"),dmy_hms(oslo_time,tz="Europe/Oslo"))
two_times <- taiwan_times
two_times
```
...
```{r ans23}
with_tz(two_times, tzone="America/Chicago") -> two_times_chicago
two_times_chicago
```
...
```{r ans41}
fiveReceipts <- c(9,7,4,4,4)
fiveReceipts.factor <- factor(fiveReceipts)
levels(fiveReceipts.factor) -> "levels_fiveReceipts"
levels_fiveReceipts
```
### 4.2
使用as.integer()函數來顯示fiveReceipts在電腦的存法，並把回傳的結果值存在storage_fiveReceipts. 
```{r ans42}
as.integer(fiveReceipts.factor) -> "storage_fiveReceipts"
...
```{r ans43}
as.character(fiveReceipts.factor) -> "chr_fiveReceipts"
chr_fiveReceipts
```
### 4.4
若我們想要把fiveReceipts的螢幕顯示值用sum()函數加總，在只使用as.{class name}函數下，要怎麼做？把sum完的結果存在sum_fiveReceipts。
```{r ans44}
sum_fiveReceipts <- sum(as.integer(as.character(fiveReceipts.factor)))
sum_fiveReceipts
```
